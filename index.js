...
app.post('/webhook', async (req, res) => {
  try {
    ...
    // Ignora status "pending"
    if (statusPagamento === 'pending') {
      console.log(`Pagamento ainda pendente para ${paymentUrl}. Nenhuma ação realizada.`);
      return res.status(200).send('Status pendente — webhook recebido, mas sem ação.');
    }

    // Só continua se status for "paid"
    if (statusPagamento !== 'paid') {
      console.log(`Status "${statusPagamento}" não tratado. Nenhuma ação realizada.`);
      return res.status(200).send('Status não tratado — webhook recebido.');
    }
    ...
    // Atualiza o campo "Pagamento" com "Confirmado"
    await axios.patch(
    ...
    console.log(`Registro ${recordId} atualizado com Pagamento: Confirmado`);
